...
if [ ! -d $tmpDir ]; then # If tmp directory doesn't exist (likely) then create it.
	mkdir -p $tmpDir # -p forces the entire path to be created otherwise if /tmp/$USER doesn't exist epic failure ensues
fi
...
ls $inputDir | grep .vcf.gz$ > fileOfFiles.txt
...
for bed in $arrIndexBedFiles; do
	...
	$(cat inputGVCF.txt) \
	-o $tmpDir/$bed.$OUTPREFIX.snps.vcf \
	-nt 2 > $WORKDIR/$SLURM_JOBID/$bed.$OUTPREFIX.pipeline.log 2>&1 &
done
wait

cat $WORKDIR/$SLURM_JOBID/*.log >> $WORKDIR/$OUTPREFIX.pipeline.log
ls | grep $OUTPREFIX.snps.vcf$ > $OUTPREFIX.vcf.list.txt
